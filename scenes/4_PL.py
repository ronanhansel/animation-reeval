...
class Scene2(Scene):
    ...
    def construct(self):
        ...
        upper_original_curve = always_redraw(lambda: axes.plot(
            lambda theta: icc_function(theta, a=2.5, b=b_tracker_2.get_value()),
            color=YELLOW,
            stroke_width=4,
            x_range=[x_tracker.get_value(), 4]
        ))
        ...
        # Morph formula to dynamic y-axis label and remove title
        self.play(ReplacementTransform(pl_1_formula, dynamic_y_label), FadeOut(title_1pl), run_time=0.8)
        self.wait(0.5)

        # Fade in the curve
        self.play(FadeIn(original_curve, axes, x_label), run_time=1.5)
        ...
        self.play(Create(upper_original_curve), run_time=2)
        self.play(x_tracker.animate.set_value(2), b_tracker_2.animate.set_value(2), run_time=2)
...
class Scene3(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(title_2pl), Write(pl_2_formula))
        self.wait(1.0)

        # Morph formula to dynamic y-axis label and remove title
        self.play(ReplacementTransform(pl_2_formula, dynamic_y_label), FadeOut(title_2pl), run_time=0.8)
        self.wait(0.5)
...
class Scene4(Scene):
    ...
    def construct(self):
        ...
        # 3-PL formula (from PL_forms) and title
        ...
        self.play(Write(title_3pl), Write(pl_3_formula))
        self.wait(1.0)

        # Morph formula to dynamic y-axis label and remove title
        self.play(ReplacementTransform(pl_3_formula, dynamic_y_label), FadeOut(title_3pl), run_time=0.8)
        self.wait(0.5)

        # Fade in the curve then show c line and label
        self.play(FadeIn(icc_curve, axes, x_label), run_time=1.5)
        self.wait(0.5)
